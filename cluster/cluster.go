...
func (c *idxCluster) Volume() float32 {
	return float32(len(c.members)) * c.rate
}

func (c *idxCluster) toCluster(a *arena.Arena, matrix MergerList) Cluster {
	templateArena := matrix.MergeAll(a, c.members)
	result := Cluster{
		Arena:         c.arena,
	...
	//result.Volume = smallestVolume * result.Rate * float32(len(c.members))
...
func (c *idxCluster) rateCandidate(candidateIdx int) float32 {
	lowestVal := c.matrix.Get(c.members[0], candidateIdx)
	for _, memberIdx := range c.members[1:] {
		v := c.matrix.Get(memberIdx, candidateIdx)
		if v < lowestVal {
			lowestVal = v
		}
	}
...
func (c *idxCluster) nextCandidate() (float32, int) {
	var maxCandidateRate float32 = .0
	maxCandidateIdx := -1

	candidateIndexes := c.matrix.Candidates(c.members)
...
func (c *idxCluster) tryAdd(candidateRate float32, candidateIndex int) bool {
	if c.Volume() < candidateRate*float32(len(c.members)+1) {
...
// 	matrix := NewRateMatrix(len(arena.List), len(arena.List), func(i, j int) float32 {
// 		if j <= i {
// 			return 0
// 		}
...
func ExtractOptimized(arena *arena.Arena) *Matrix {
	...
	matrix := NewOptimizedRateMatrixAsync(len(arena.List), len(arena.List), 4, func(i, j int) float32 {
		// matrix := NewOptimizedRateMatrix(len(arena.List), len(arena.List), func(i, j int) float32 {
		// matrix := NewRateMatrix(len(arena.List), len(arena.List), func(i, j int) float32 {
		if j <= i {
			return 0
		}
	...
	for {
		...
		for nextVal, nextIndex := icluster.nextCandidate(); nextIndex > -1; nextVal, nextIndex = icluster.nextCandidate() {
			...
			for _, idx := range icluster.members {
				matrix.Exclude(nextIndex, idx)
			}
		}
		for _, idx := range icluster.members {
			matrix.ExcludeRow(idx)
		}
		idxClusters = append(idxClusters, icluster)
	}

	// trace
	fmt.Println("cluster grouped in:", time.Since(startedAt).Seconds(), "seconds")
	startedAt = time.Now()

	clusters := make([]Cluster, len(idxClusters))
	for i := range idxClusters {
		clusters[i] = idxClusters[i].toCluster(arena, matrixCopy)
	}

	fmt.Println("cluster merged in:", time.Since(startedAt).Seconds(), "seconds")
	startedAt = time.Now()
...
func (c *Cluster) TemplateVolume() float32 {
	var vol float32 = .0
